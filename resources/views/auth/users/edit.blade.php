...
@section('title', 'Edit User - logistics')
@section('page-title', 'Edit User: ' . $user->name)

@section('content')
    <div style="margin-bottom: 20px;">
        <a href="{{ route('auth.users.index') }}" class="btn btn-secondary">← Back to Users</a>
        <a href="{{ route('auth.users.show', $user) }}" class="btn btn-primary">View User</a>
    </div>

    {{-- 🔥 UPDATE FORM (SEPARATE) --}}
    <form method="POST" action="{{ route('auth.users.update', $user) }}" id="updateUserForm">
        @csrf
        @method('PUT')
    ...
    @if ($user->id !== auth()->id())
        <div style="margin-top: 30px; padding-top: 20px; border-top: 2px solid #fee2e2;">
            ...
            <form method="POST" action="{{ route('auth.users.destroy', $user) }}" id="deleteUserForm"
