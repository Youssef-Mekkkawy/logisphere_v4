...
@section('content')
    <div style="margin-bottom: 2rem;">
        <h1 style="font-size: 1.875rem; font-weight: 700; color: #1e293b; margin-bottom: 0.5rem;">Edit Shipping Agency</h1>
        <p style="color: #64748b;">Update shipping agency information and details</p>
    </div>

    <div class="card">
        <div class="card-header">
            <h3>Edit Agency - {{ $shippingAgency->name }}</h3>
        </div>
        <div class="card-body">
            <form action="{{ route('logistics.shipping-agencies.update' ?? '', $shippingAgency) }}" method="POST">
                @csrf
                @method('PUT')
                ...
                <div class="form-grid">
                    <div class="form-group">
                        ...
                        <input type="text" id="code" name="code"
                            ...
                            value="{{ old('code', $shippingAgency->code) }}" required placeholder="e.g., MSK, COSCO, EVER">
                        @error('code')
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" id="name" name="name"
                            ...
                            value="{{ old('name', $shippingAgency->name) }}" required
                        ...
                        <select id="country_id" name="country_id" class="form-input @error('country_id') error @enderror"
                            ...
                            @foreach ($countries as $country)
                                <option value="{{ $country->id }}"
                                    {{ old('country_id', $shippingAgency->country_id) == $country->id ? 'selected' : '' }}>
                                    {{ $country->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="form-group">
                        ...
                        <select id="service_type" name="service_type"
                            ...
                            <option value="Ocean Freight"
                                {{ old('service_type', $shippingAgency->service_type) == 'Ocean Freight' ? 'selected' : '' }}>
                                Ocean Freight</option>
                            <option value="Air Freight"
                                {{ old('service_type', $shippingAgency->service_type) == 'Air Freight' ? 'selected' : '' }}>
                                Air Freight</option>
                            <option value="Land Transport"
                                {{ old('service_type', $shippingAgency->service_type) == 'Land Transport' ? 'selected' : '' }}>
                                Land Transport</option>
                            <option value="Full Service"
                                {{ old('service_type', $shippingAgency->service_type) == 'Full Service' ? 'selected' : '' }}>
                                Full Service</option>
                        </select>
                        @error('service_type')
                            <span class="error-message">{{ $message }}</span>
                ...
                <div class="form-grid">
                    <div class="form-group">
                        ...
                        <input type="text" id="contact_person" name="contact_person"
                            ...
                            value="{{ old('contact_person', $shippingAgency->contact_person) }}"
                    ...
                    <div class="form-group">
                        ...
                        <input type="email" id="email" name="email"
                            ...
                            value="{{ old('email', $shippingAgency->email) }}" placeholder="agency@example.com">
                        @error('email')
                            <span class="error-message">{{ $message }}</span>
                        @enderror
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" id="phone" name="phone"
                            ...
                            value="{{ old('phone', $shippingAgency->phone) }}" placeholder="+1234567890">
                        @error('phone')
                            <span class="error-message">{{ $message }}</span>
                        @enderror
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <select id="status" name="status" class="form-input @error('status') error @enderror" required>
                            <option value="Active"
                                {{ old('status', $shippingAgency->status) == 'Active' ? 'selected' : '' }}>Active</option>
                            <option value="Inactive"
                                {{ old('status', $shippingAgency->status) == 'Inactive' ? 'selected' : '' }}>Inactive
                            </option>
                        </select>
                        @error('status')
                            <span class="error-message">{{ $message }}</span>
                ...
                <div class="form-group">
                    ...
                    <textarea id="address" name="address" class="form-input @error('address') error @enderror" rows="3"
                        placeholder="Enter agency address">{{ old('address', $shippingAgency->address) }}</textarea>
                    @error('address')
                        <span class="error-message">{{ $message }}</span>
                    @enderror
                </div>
                ...
                <div class="form-group">
                    ...
                    <textarea id="services_offered" name="services_offered" class="form-input @error('services_offered') error @enderror"
                        rows="4" placeholder="Describe the services this agency provides...">{{ old('services_offered', $shippingAgency->services_offered) }}</textarea>
                ...
                <div style="margin-top: 2rem; display: flex; gap: 1rem;">
                    ...
                    <a href="{{ route('logistics.shipping-agencies.show' ?? '', $shippingAgency) }}"
