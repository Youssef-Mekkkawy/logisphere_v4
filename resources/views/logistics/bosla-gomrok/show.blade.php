...
@section('content')
    ...
    <div style="margin-top: 2rem; display: flex; gap: 1rem;">
        <a href="{{ route('logistics.bosla-gomrok.edit' ?? '' , $boslaGomrok) }}" class="btn btn-primary">Edit Bosla</a>
        <a href="{{ route('logistics.bosla-gomrok.index') ?? ''  }}" class="btn btn-secondary">Back to Bosla List</a>
        @if (auth()->user() && method_exists(auth()->user(), 'isAdmin') && auth()->user()->isAdmin())
            <form action="{{ route('logistics.bosla-gomrok.destroy' ?? '' , $boslaGomrok) }}" method="POST"
