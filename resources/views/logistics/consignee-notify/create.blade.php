...
@section('title', 'Add New Consignee/Notify Party')

@section('content')
    <div style="margin-bottom: 2rem;">
        <h1 style="font-size: 1.875rem; font-weight: 700; color: #1e293b; margin-bottom: 0.5rem;">Add New Consignee/Notify
            Party</h1>
        <p style="color: #64748b;">Create a new party for shipment delivery and notifications</p>
    </div>

    <form action="{{ route('logistics.consignee-notify.store') }}" method="POST">
        @csrf
            ...
            <div class="card-body">
                <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(300px, 1fr)); gap: 1.5rem;">
                    <div class="form-group">
                        ...
                        <input type="text" name="party_code" class="form-input" required value="{{ old('party_code') }}"
                            placeholder="e.g., CNS001">
                        @error('party_code')
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="party_name" class="form-input" required value="{{ old('party_name') }}"
                    ...
                    <div class="form-group">
                        ...
                        <select name="party_type" class="form-input" required>
                            ...
                            <option value="Consignee" {{ old('party_type') == 'Consignee' ? 'selected' : '' }}>Consignee
                            </option>
                            <option value="Notify Party" {{ old('party_type') == 'Notify Party' ? 'selected' : '' }}>Notify
                                Party</option>
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="company_registration" class="form-input"
                            value="{{ old('company_registration') }}" placeholder="Registration number">
            ...
            <div class="card-body">
                <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(300px, 1fr)); gap: 1.5rem;">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="fax" class="form-input" value="{{ old('fax') }}"
                    ...
                    <div class="form-group">
                        ...
                        <select name="preferred_language" class="form-input" required>
                            ...
                            <option value="ar" {{ old('preferred_language') == 'ar' ? 'selected' : '' }}>Arabic
                            </option>
                            <option value="fr" {{ old('preferred_language') == 'fr' ? 'selected' : '' }}>French
                            </option>
                            <option value="es" {{ old('preferred_language') == 'es' ? 'selected' : '' }}>Spanish
                            </option>
                        </select>
                    ...
                    <div style="display: flex; gap: 1rem; flex-wrap: wrap;">
                        <label style="display: flex; align-items: center; gap: 0.5rem;">
                            <input type="checkbox" name="notification_preferences[]" value="email"
                                {{ in_array('email', old('notification_preferences', [])) ? 'checked' : '' }}>
                            Email Notifications
                        </label>
                        <label style="display: flex; align-items: center; gap: 0.5rem;">
                            <input type="checkbox" name="notification_preferences[]" value="sms"
                                {{ in_array('sms', old('notification_preferences', [])) ? 'checked' : '' }}>
                            SMS Notifications
                        </label>
                        <label style="display: flex; align-items: center; gap: 0.5rem;">
                            <input type="checkbox" name="notification_preferences[]" value="phone"
                                {{ in_array('phone', old('notification_preferences', [])) ? 'checked' : '' }}>
                            Phone Notifications
                        </label>
                    </div>
                </div>
            ...
            <div class="card-body">
                <div class="form-group" style="margin-bottom: 1.5rem;">
                    ...
                    <textarea name="address" class="form-input" required rows="3"
                        placeholder="Street address, building number, etc.">{{ old('address') }}</textarea>
                    @error('address')
                        <span class="error-message">{{ $message }}</span>
                    @enderror
                </div>
                ...
                <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(300px, 1fr)); gap: 1.5rem;">
                    <div class="form-group">
                        ...
                        <select name="credit_rating" class="form-input">
                            ...
                            <option value="B" {{ old('credit_rating') == 'B' ? 'selected' : '' }}>B - Good</option>
                            <option value="C" {{ old('credit_rating') == 'C' ? 'selected' : '' }}>C - Fair</option>
                            <option value="D" {{ old('credit_rating') == 'D' ? 'selected' : '' }}>D - Poor</option>
                        </select>
                        @error('credit_rating')
                            <span class="error-message">{{ $message }}</span>
                        @enderror
                    ...
                    <div class="form-group">
                        <label style="display: flex; align-items: center; gap: 0.5rem;">
                            <input type="checkbox" name="requires_original_docs" value="1"
                                {{ old('requires_original_docs', '1') ? 'checked' : '' }}>
                    ...
                    <div class="form-group">
                        <label style="display: flex; align-items: center; gap: 0.5rem;">
                            <input type="checkbox" name="is_freight_forwarder" value="1"
                                {{ old('is_freight_forwarder') ? 'checked' : '' }}>
            ...
            <div class="card-body">
                ...
                <div class="form-group" style="margin-bottom: 1.5rem;">
                    ...
                    <textarea name="special_requirements" class="form-input" rows="3"
                        placeholder="Any special handling requirements...">{{ old('special_requirements') }}</textarea>
                ...
                <div class="form-group">
                    ...
                    <textarea name="notes" class="form-input" rows="3" placeholder="Additional notes...">{{ old('notes') }}</textarea>
                    @error('notes')
                        <span class="error-message">{{ $message }}</span>
                    @enderror
                </div>
        ...
        <div style="display: flex; gap: 1rem; justify-content: flex-end;">
            <a href="{{ route('logistics.consignee-notify.index') }}" class="btn btn-secondary">Cancel</a>
