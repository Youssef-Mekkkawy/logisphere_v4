...
@section('title', 'Create Shipment - LogiFlow')
    ...
    <div class="form-grid">
        ...
        <div class="form-group">
            ...
            <select name="container_type" class="form-input">
                ...
                <option value="20ft Container" {{ old('container_type') == '20ft Container' ? 'selected' : '' }}>20ft Container</option>
                <option value="40ft Container" {{ old('container_type', '40ft Container') == '40ft Container' ? 'selected' : '' }}>40ft Container</option>
                <option value="40ft High Cube" {{ old('container_type') == '40ft High Cube' ? 'selected' : '' }}>40ft High Cube</option>
                <option value="45ft Container" {{ old('container_type') == '45ft Container' ? 'selected' : '' }}>45ft Container</option>
            </select>
        ...
        <div class="form-group">
            ...
            <input type="number" name="freight_cost" class="form-input" step="0.01" 
                   placeholder="0.00" value="{{ old('freight_cost') }}">
            @error('freight_cost')
                <span class="error-message">{{ $message }}</span>
            @enderror
        </div>
        ...
        <div class="form-group" style="grid-column: 1 / -1;">
            ...
            <textarea name="cargo_description" class="form-input" rows="3" 
                      placeholder="Describe the cargo being shipped...">{{ old('cargo_description') }}</textarea>
...
<div style="margin-top: 30px; padding: 20px; background: #fef3c7; border: 1px solid #f59e0b; border-radius: 10px; color: #92400e;">
    <h4 style="margin-bottom: 10px;">⚠️ Setup Required</h4>
    <p style="margin-bottom: 15px;">To create shipments, you need:</p>
    <ul style="margin: 0; padding-left: 20px;">
        @if($companies->isEmpty())
            <li>At least one company. <a href="{{ route('management.companies.create') }}" style="color: #1e40af;">Create a company</a></li>
        @endif
        @if($ports->isEmpty())
            <li>At least two ports. <a href="{{ route('logistics.ports.index') }}" style="color: #1e40af;">Manage ports</a></li>
        @endif
    </ul>
</div>
@endif
